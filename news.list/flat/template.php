...
$this->addExternalJs($this->GetFolder().'/themes/like.js');
...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	<div class="bx-newslist-block">
		...
		<div class="row">
			<div class="col-xs-5">
			<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
				<div class="bx-newslist-more"><a class="btn btn-primary btn-xs" href="<?echo $arItem["DETAIL_PAGE_URL"]?>"><?echo GetMessage("CT_BNL_GOTO_DETAIL")?></a>
                <br>
                    <?
                    if (in_array($_SESSION['SESS_AUTH']['USER_ID'], $arItem["PROPERTIES"]["LIKE"]["VALUE"])){
                        echo "<a class='like' do='no'  attr='".$arItem["ID"]."'>Уже не нравится</a>";
                    }
                    else {
                        echo "<a class='like' do='yes'  attr='".$arItem["ID"]."'>Мне нравится</a>";
                    };
...
if ($_POST['like'] == "1") {
    ...
    $res = CIBlockElement::GetProperty(1, $_POST['element'], "sort", "asc", array("CODE" => "LIKE"));
    while ($ob = $res->GetNext())
    {
        $PROPERTY_VALUE[] = $ob['VALUE'];
    }
    $ELEMENT_ID = $_POST['element'];
    $PROPERTY_CODE = "LIKE";
    array_push($PROPERTY_VALUE, $_SESSION['SESS_AUTH']['USER_ID']);
    CIBlockElement::SetPropertyValuesEx($ELEMENT_ID, 1, array($PROPERTY_CODE => $PROPERTY_VALUE));
    BXClearCache(true, "/news/");
}
elseif ($_POST['dislike'] == "1") {
    $PROPERTY_VALUE = array();
    $res = CIBlockElement::GetProperty(1, $_POST['element'], "sort", "asc", array("CODE" => "LIKE"));
    while ($ob = $res->GetNext()) {
        $PROPERTY_VALUE[] = $ob['VALUE'];
    ...
    if (count($PROPERTY_VALUE) != 1){
        unset($PROPERTY_VALUE[array_search($_SESSION['SESS_AUTH']['USER_ID'], $PROPERTY_VALUE)]);
    }
    else {
